...
- **Physical collection** — **COLLECTION** tab for owned editions (cards/list, filters, Excel import/export). **Add to collection** on release tracklist editions; checklist of Cover / Photo / Photocards / Codes / Media / Branding; **Manage collection** in the hamburger while the tab is active. Preferred code artwork filenames: `Code - Spotify`, `Code - Spotify Card`, `Code - QR`, `Code - QR Card` (legacy `Spotify - *` / `QR - *` still detected).
