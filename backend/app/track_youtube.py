...
from app.track_overrides import read_youtube_url, save_youtube_url, youtube_overrides_map
        ...
        for group in edition.get("groups") or []:
            for track in group.get("tracks") or []:
                ...
                if not play_path:
                    ...
                    continue
                track["youtube_url"] = _resolve_track_youtube(
                    db,
                    play_path=play_path,
                    title=title,
                    youtube_map=youtube_map,
...
def read_track_youtube(
    ...
    *,
    ...
    if play_path:
        ...
        if url:
            source = "db" if play_path in path_overrides or read_youtube_url(db, play_path) else "legacy"
            return url, source
    url = _lookup_youtube(youtube_map, title)
    return url, "db" if url else "none"
